...
interface SizingTier {
  ...
  compute: { instance: string; count: number; hourly: number };
  rds: { instance: string; hourly: number; storageGb: number; replicas: number };
  cache: { node: string; count: number; hourly: number };
}

function sizeForUsers(users: number): SizingTier {
  if (users < 50000) {
    return {
      tier: "small",
...
interface CostLineItem {
  sku: string;
  category: "compute" | "storage" | "network" | "mgmt";
  amount: number;
}

function computeCost(users: number): {
  items: CostLineItem[];
  total: number;
  sizing: SizingTier;
} {
...
// ============================================================
function PatternCandidatesInline({ candidates }: { candidates: PatternCandidate[] }) {
  return (
...
function ArchitectureDiagram({
  ...
  type DiagramBox = (typeof components)[number];
  const compById = Object.fromEntries(components.map((c: DiagramBox) => [c.id, c])) as Record<string, DiagramBox>;
  const center = (c: DiagramBox) => ({ x: c.x + c.w / 2, y: c.y + c.h / 2 });
...
// COST PANEL
// ============================================================
type PricingTerm = "on_demand" | "reserved_1y";
...
function CostPanel({
  ...
  populated: boolean;
  term: PricingTerm;
  setTerm: (t: PricingTerm) => void;
}) {
  const cost = computeCost(users);
  const annual = cost.total * 12;
  const reservedSavings = cost.total * 0.38;

  const byCategory: Record<CostLineItem["category"], number> = {
    compute: 0,
    storage: 0,
    network: 0,
    mgmt: 0,
  };
  for (const i of cost.items) {
      ...
      <div className="px-5 pt-5 pb-4 border-b border-zinc-200">
        <div className="flex items-baseline justify-between mb-1">
          ...
          <div className="flex gap-0.5 text-2xs font-mono">
            {(["on_demand", "reserved_1y"] as const).map(t => (
              <button
                key={t}
                onClick={() => setTerm(t)}
...
function PrintDiagramContent({ sizing }: { sizing: SizingTier }) {
  type PrintBox = {
    id: string;
    x: number;
    y: number;
  ...
  const compById = Object.fromEntries(components.map(c => [c.id, c])) as Record<string, PrintBox>;
  const center = (c: PrintBox) => ({ x: c.x + c.w / 2, y: c.y + c.h / 2 });

  return (
    <>
      <rect x="20" y="240" width="470" height="170" rx="6" fill="none" stroke="#a1a1aa" strokeWidth="1" strokeDasharray="3,3" />
      <text x="32" y="234" fill="#71717a" fontSize="9" fontFamily="JetBrains Mono">
        VPC · us-east-1 · 2 AZs
      </text>
...
export default function App() {
  ...
  const [diagramView, setDiagramView] = useState<"diagram" | "rationale" | "flow">("diagram");
            ...
            {chatItems.map((m, i) => (
              <div key={i} className={`fade-in-up ${m.role === "user" ? "" : ""}`}>
                ...
                {m.role === "agent" && i === chatItems.length - 1 && stage === "running_tools" && (
                  <div className="mt-3">
                    {toolStatuses.map(s => {
                      const tc = SCENARIO.toolCalls[s.idx];
                      if (!tc) return null;
                      return (
                        <ToolCallCard
                          key={s.idx}
            ...
            <div className="flex items-center gap-1">
              {(
                [
                  ...
                  { id: "rationale" as const, label: "Rationale", disabled: !populated },
                  { id: "flow" as const, label: "Flow", disabled: !populated },
                ] as const
              ).map(t => (
                <button
                  key={t.id}
                  onClick={() => !("disabled" in t && t.disabled) && setDiagramView(t.id)}
